...
public class MyFundingServiceImpl implements MyFundingService{
    ...
    @Override
    public List<Funding> findFundingByMember(Long member_id){
        log.info("Find Funding by member_Id {}",member_id);
        return fundingRepo.findByMember_id(member_id);
    }

    @Override
    public ResponseEntity<List<ReturnFundingInfo>> getFundingById(List<Funding> fundingList) {

        log.info("##fundingList is : {}", fundingList);
        List<Long> fundingByMemberId = fundingList.stream().map(funding-> {
