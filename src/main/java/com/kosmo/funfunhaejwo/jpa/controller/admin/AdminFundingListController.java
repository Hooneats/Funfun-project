...
import com.kosmo.funfunhaejwo.jpa.service.DeliveryService;
...
public class AdminFundingListController {
    ...
    void updateStatus(@PathVariable("id") long funding_id, @PathVariable("status") ProductPaymentStatus status) {
        productPaymentRepo.update(funding_id, status);
    }

    @PostMapping("/update/shipping")
    void updateShipping(@RequestParam long funding_id, @RequestParam String delivery_num) {
        deliveryService.insertDelivery(funding_id, delivery_num);
    }
}
